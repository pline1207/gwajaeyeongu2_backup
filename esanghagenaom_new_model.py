...
def spherical_collate_longitudinal(batch):
    # batch: list of (data(C,N), name, time_obs, subject_id, ref_data)
...
class SPHARM_Net(nn.Module):
    def __init__(self, sphere, device, in_ch=1, n_class=1, C=128, L=80, D=3, interval=5, 
        ...
        # 🌟 [ViViT] 4. ViViT Reference Embedding ---
        self.use_ref_condition = use_ref_condition
        # [수정] 나중에 접근할 수 있도록 멤버 변수에 저장
        self.ref_num_frames = ref_num_frames if ref_num_frames else 1
        
        if self.use_ref_condition:
            # ViViT 초기화: ref_img의 spatial + temporal 특징을 모두 잡기 위해 사용
            self.vivit = ViViT(
                image_size = L,          # SPHARM L 또는 vertex 수 (ViViT 구현에 따름)
                patch_size = 16,         
                ...
                num_frames = self.ref_num_frames # 저장된 변수 사용
            )
        ...
        if num_subjects is not None:
            combined_dim_in += time_dim 
        if self.use_ref_condition:
            combined_dim_in += time_dim # Ref Emb 추가
    ...
    def forward(self, x, t=None, x_self_cond=None, time_obs=None, subject_id=None, ref_img=None):
        ...
        # 🌟 [ViViT] 4. ViViT Reference Embedding (수정된 부분)
        if self.use_ref_condition:
            if ref_img is not None:
                # ref_img shape 기대값: (B, T, C, N) -> 여기서 T=2
                x_ref = ref_img
                ...
                # 차원 확인 및 보간 (Interpolation)
                # 입력이 (B, C, N)인 경우 (혹시 모를 하위 호환성) -> (B, 1, C, N)으로 확장
                if x_ref.ndim == 3:
                    x_ref = x_ref.unsqueeze(1)

                # 이제 x_ref는 (B, T, C, N) 형태라고 가정
                b, t, c, n = x_ref.shape

                # 보간을 위해 (B*T, C, N)으로 병합
                x_ref_flat = x_ref.view(b * t, c, n)

                # 데이터 길이(N)가 ViViT 입력(L*L)과 안 맞으면 보간
                if n != target_area:
                    x_ref_flat = F.interpolate(x_ref_flat, size=target_area, mode='linear', align_corners=False)
                
                # 정사각형 이미지로 변환: (B*T, C, L, L)
                x_ref_2d = x_ref_flat.view(b * t, c, target_size, target_size)

                # 다시 시간 차원 복구: (B, T, C, H, W)
                x_ref_final = x_ref_2d.view(b, t, c, target_size, target_size)
                # ---------------- [수정된 로직 끝] ----------------
                
                # ViViT Forward
                ref_emb = self.vivit(x_ref_final) # (B, time_dim)
...
class GaussianDiffusion(Module):
    ...
    def model_predictions(self, x, t, x_self_cond = None, clip_x_start = False, rederive_pred_noise = False, 
                          time_obs = None, subject_id = None, ref_img = None, **kwargs):
        # 🌟 ViViT를 위해 ref_img 전달
    ...
    def p_sample(self, x, t: int, x_self_cond = None, 
        ...
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(
            ...
            time_obs=time_obs, subject_id=subject_id, ref_img=ref_img, **kwargs
        )
    ...
    def q_sample(self, x_start, t, noise = None):
        ...
        if self.immiscible:
            # simple noise assignment for now, can be optimized
            pass
        return (
    ...
    def p_losses(self, x_start, t, noise = None, offset_noise_strength = None, 
        ...
        # Geometry Regularizers (Cycle, Laplacian, Normal)
    ...
    def forward(self, data_batch, *args, **kwargs):
        # data_batch handling: (img, T_obs, S_id, Ref_img, names...)
        try:
            img = data_batch[0]
...
class SphericalTrainer:
    ...
    def _sample_and_save(self):
        ...
        subject_id = batch[2][:B].to(self.device) # ✅ 여기서 subject_id로 정의됨
        ...
        print(f"\n--- Sampling Start (Milestone {milestone}) using REAL Data ---")
        ...
        # -------------------------------------------------------------
        # 4. 메타데이터 저장 (.txt) -> 🚨 여기가 수정된 부분입니다
        # -------------------------------------------------------------
        # 기존: if subject_ids_int is not None ... (에러 발생 원인)
        # 수정: if subject_id is not None ...
        if subject_id is not None and hasattr(self.ds, 'subject_to_id'):
            ...
            with open(save_path_txt, 'w') as f:
                f.write("Index\tSubject_Name\tAge(Time)\tRef_Used\n")
                for i in range(B):
                    # 이름 리스트가 있으면 이름 사용, 없으면 ID 사용
                    subj_name = names_list[i] if len(names_list) > i else str(s_ids_list[i])
                    t_val = t_obs_list[i]
                    ref_status = "Yes" if ref_img is not None else "No"
                    
                    f.write(f"{i}\t{subj_name}\t{t_val:.4f}\t{ref_status}\n")
