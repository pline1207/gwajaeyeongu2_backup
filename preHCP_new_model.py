...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None):
        h = self.heads
        # SPHARM Feature (Query)
        # x shape: (B, C, L) -> permute -> (B, L, C)
        x_perm = x.permute(0, 2, 1)
        
        q = self.to_q(x_perm)
        
        # Context (Key, Value) from ViViT
        context = default(context, x_perm)
        k = self.to_k(context)
        v = self.to_v(context)
...
class SpatialTransformer(nn.Module):
    def __init__(self, dim, context_dim, heads=8, dim_head=64, dropout=0.):
        ...
        # Cross Attention
        self.attn = CrossAttention(query_dim=dim, context_dim=context_dim, heads=heads, dim_head=dim_head, dropout=dropout)
        
        # Feed Forward
        self.norm_ff = nn.GroupNorm(8, dim)
    ...
    def forward(self, x, context=None):
        ...
        # 1. Cross Attention
        x = self.norm(x)
        x = self.attn(x, context=context) + residue
...
class SPHARM_Net(nn.Module):
    def __init__(self, sphere, device, in_ch=1, n_class=1, C=128, L=80, D=3, interval=5, 
        ...
        self.context_dim = 512 # ViViT default dim
        
        if self.use_ref_condition:
            self.vivit = ViViT(
                image_size = L,            
                patch_size = 16,           
                num_classes = self.context_dim, 
        ...
        # Global MLP for time/subj/obs embeddings
        ...
        for i in range(D):
            ...
            if self.use_ref_condition:
                self.down_attns.append(
                    SpatialTransformer(dim=out_ch, context_dim=self.context_dim, heads=4, dim_head=32)
                )
        ...
        if self.use_ref_condition:
            self.mid_attn = SpatialTransformer(dim=out_ch, context_dim=self.context_dim, heads=8, dim_head=64)
    ...
    # [중요] Pretrained Weight Loading Helper Function
    def load_pretrained_weights(self, path, device):
        ...
        # Filter out size-mismatched keys (mainly 'subject_emb.weight')
        # HCP(Subject 많음) -> OASIS(Subject 적음) 이동 시 에러 방지
        pretrained_dict = {k: v for k, v in state_dict.items() if k in model_dict and v.size() == model_dict[k].size()}
        
        skipped = set(state_dict.keys()) - set(pretrained_dict.keys())
        if skipped:
            print(f"Warning: Skipped loading layers due to size mismatch (likely subject_emb): {len(skipped)} keys skipped.")
    ...
    def forward(self, x, t=None, x_self_cond=None, time_obs=None, subject_id=None, ref_img=None):
        ...
        # 1. Embeddings (Time, Subject)
        if t is None: t = torch.zeros(x.shape[0], device=x.device, dtype=torch.long)
        ...
        if self.max_time_obs is not None and time_obs is not None:
            ...
            cond_embs.append(self.time_obs_mlp(norm_time_obs))

        if self.num_subjects is not None and subject_id is not None:
            cond_embs.append(self.subject_emb(subject_id))
        ...
        # 2. ViViT Feature Extraction
        context = None
        if self.use_ref_condition and ref_img is not None:
            x_ref = ref_img
            if x_ref.ndim == 3: x_ref = x_ref.unsqueeze(1) # (B, 1, C, N)
            
            b, t_ref, c, n = x_ref.shape
            target_size = self.vivit_image_size 
            ...
            vivit_out = self.vivit(x_ref_final)
            
            if vivit_out.ndim == 2:
                context = vivit_out.unsqueeze(1)
            else:
                context = vivit_out
        ...
        for i, blk in enumerate(self.down):
            ...
            if self.use_ref_condition and context is not None:
                y = self.down_attns[i](y, context=context)
                
            feats.append(y)

        y = self.bottom(y, time_emb=time_emb)
        if self.use_ref_condition and context is not None:
            y = self.mid_attn(y, context=context)
        ...
        for i, blk in enumerate(self.up):
            ...
            if self.use_ref_condition and context is not None:
                y = self.up_attns[i](y, context=context)
...
class SphericalTrainer:
    ...
    def _sample_and_save(self):
        ...
        if len(batch) == 5: 
             ...
             names_list = batch[4][:B]
        elif len(batch) == 4:
             names_list = batch[3][:B]
        else:
             names_list = []
